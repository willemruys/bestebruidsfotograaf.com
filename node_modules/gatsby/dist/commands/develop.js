...
module.exports = async program => {
  const developProcessPath = require.resolve(`./develop-process`); // Run the actual develop server on a random port, and the proxy on the program port
  // which users will access


  const proxyPort = program.port;
  const developPort = await (0, _detectPort.default)();
  ...
  const statusServerPort = await (0, _detectPort.default)();
  let unlock;

  if (!(0, _gatsbyCoreUtils.isCI)()) {
    unlock = await (0, _serviceLock.createServiceLock)(program.directory, `developstatusserver`, statusServerPort.toString());
